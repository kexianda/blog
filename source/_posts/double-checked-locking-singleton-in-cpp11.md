...
1. 了解CPU的Memory Barriers机制(Paul McKenny的[Memory Barriers: a Hardware View for Software Hackers][1])。了解CPU内部不同的核是怎么同步cache的，很有意思。这是理解memory order的基础。
2. 然后看Jeff Preshing的[blog](http://preshing.com), 他的帖子深入浅出，写得非常好。
3. 《C++ Concurrency in action》, 作者是boost thread的作者。讲stl的thread的接口。
4. 再看看Herb Sutter,Hans Boehm等人的文章。 [Bartosz Milewski的博客][7]也值得看看.

double-checked locking是一个用来学习多线程的好例子。 Scott Meyers和Andrei Alexandrescu两位大牛写过一篇[paper][2]，讨论了double-checked实现的困难(Java的memeory model没有完善之前有同样的问题). Jeff Preshing写篇[文章][3]讨论这个问题.
